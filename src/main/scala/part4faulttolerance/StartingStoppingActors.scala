...
object StartingStoppingActors extends App {
  ...
  //  for (i <- 1 to 100) child2 ! s"[$i] second kind, are you still alive?"
...
//  abruptlyTerminatedActor ! "you are about the be terminated"
//  abruptlyTerminatedActor ! Kill
//  abruptlyTerminatedActor ! "you've been terminated"
  ...
  class Watcher extends Actor with ActorLogging {
    ...
    override def receive: Receive = {
      case StartChild(name) =>
        ...
        context.watch(child)
      case Terminated(ref) =>
        log.info(s"the reference I'm watching ${ref.path.name} has been stopped")
    }
  }
